...
def test_model_range(model_start_date=None, ho_start_date=None, MODEL_WIN=None):
    ...
    """
    mod_range = pd.date_range(start=model_start_date,
                              periods=MODEL_WIN, freq='D')
    assert_eq(len(mod_range), MODEL_WIN)
    assert_eq(mod_range[0].to_pydatetime().date(), model_start_date)
    assert_eq(mod_range[-1].to_pydatetime().date() +
              dt.timedelta(days=1), ho_start_date)
...
def test_rft(df, duration=None):
    ...
    max_dur = df.eval(
        'Max_day - Min_day').astype('timedelta64[D]').astype(int).max()
